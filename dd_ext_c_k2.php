 ...
 */
class PlgDD_GMaps_LocationsDD_Ext_C_K2 extends JPlugin
{
	...
	public function onextc($results, $extc_plugin)
	{
		...
		{
			if ($app->input->getCmd('option') === 'com_k2')
			{
		 ...
		 * extc_link setup
		 *
		 * Check if com_k2 is requested
		 * */
		if ($extc_plugin === 'com_k2')
		{
			foreach ($results as $key => $result)
			{
				if ($result->ext_c_id !== '0')
				{
					$results[$key]->extc_link = 'index.php?option=com_k2&view=item&layout=item&id=' . $result->ext_c_id;
				}
			}
